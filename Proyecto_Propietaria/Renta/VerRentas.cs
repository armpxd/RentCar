...
namespace Proyecto_Propietaria
{
    public partial class VerRentas : Form
    {
        ...
        private void reload()
        {
            ...
            {
                var renta = db.Rent_Devolution.Select(
                          x => new
                          {
                              Id = x.Rent_id,
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            DetalleVerRenta detalle = new DetalleVerRenta(id);
            detalle.ShowDialog();
            reload();
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            ...
            using (RentCarEntities db = new RentCarEntities())
            {
                var renta = db.Rent_Devolution.Where(x => (x.State == true) && (x.Client.Name.Contains(textBox1.Text) || x.Car.Description.Contains(textBox1.Text)
                                                       ...
                                                       || x.Employee.Name.Contains(textBox1.Text) || x.Car.Type_fuel.Description.Contains(textBox1.Text)
                    )).Select(
                          x => new
                          {
                              Id = x.Rent_id,
